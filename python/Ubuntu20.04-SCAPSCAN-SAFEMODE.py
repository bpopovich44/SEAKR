...
class VulnScanner:
    ...
    def stig_scan(self):
        ...
        if(self.conn == "local"):
            b = self.stig_rulebook[self.vuln_id]["cmd"]
            if re.search('\|',b):
                p = b.split('|')

                j = subprocess.Popen(p[0].split(), shell=False, stdout=subprocess.PIPE)
                k = subprocess.Popen(p[1].split(),shell=False, stdin=j.stdout,stdout=subprocess.PIPE)
                for s in (str(k.communicate())[3:-10]).split('\\n'):
                    ro = s
            else:
                c = self.stig_rulebook[self.vuln_id]["cmd"].split()
               
                a = subprocess.Popen(c,shell=False,stdout=subprocess.PIPE,universal_newlines=True)
                ro=a.communicate()[0]
    ...
    def stig_rule_238215(self, ro):
        if(self.conn == "local"):
            b = self.stig_rulebook[self.vuln_id]["cmd1"]
            if re.search('\|',b):
                p = b.split('|',1)
                ...
                my = p[0].split()
                print("my is ---> ",my)
                j = subprocess.Popen(my, shell=False, stdout=subprocess.PIPE, universal_newlines=True)
                outer = j.communicate()
                print("j is -> ",outer)
                #k = subprocess.Popen(p[1].split(),shell=False, stdin=j.stdout,stdout=subprocess.PIPE)
                #outer1 = k.communicate()
                #print("k is -> ",outer1)
                #for s in (str(k.communicate())[3:-10]).split('\\n'):
                #    ro1 = s
            else:
                c = self.stig_rulebook[self.vuln_id]["cmd1"].split()
    ...
    def stig_rule_238198(self, ro):
        if re.search("No such file or directory", ro):
            stdin, stdout, stderr=self.conn.exec_command(self.stig_rulebook[self.vuln_id]["cmd1"])
            ro=''.join(stdout.readlines())
            if re.search("/usr/bin/byobu-select-session" or "/usr/bin/dbus-run-session", ro):
                return "not_finding"
    ...
    def stig_rule_238208(self, ro):
        ...
        if re.search("NOPASSWD",ro) or re.search("!authenticate", ro):
            return "finding"
        else:
            return "not_finding"
    ...
    def stig_rule_238228(self, ro):
        if(self.conn == "local"):
            ...
            c = self.stig_rulebook[self.vuln_id]["cmd2"]
            if re.search('\|',c):
                n = c.split('|')

                q = subprocess.Popen(n[0].split(), shell=False, stdout=subprocess.PIPE)
                m = subprocess.Popen(n[1].split(),shell=False, stdin=q.stdout,stdout=subprocess.PIPE)
                for r in (str(m.communicate())[3:-10]).split('\\n'):
                    ro2 = r
            else:
                c = self.stig_rulebook[self.vuln_id]["cmd2"].split()
        ...
        if(len(ro.strip())) == 0:
            return "finding"
        #if re.search("ii\s+?\S+?\s+?libpam-pwquality:amd64\s+?\S+?\s+?PAM module", ro):
        #if re.search(r
        if re.search("ii",ro) and re.search("libpam-pwquality:amd64",ro) and re.search("PAM module",ro):
            if re.search("enforcing" and "1", ro):
                if(len(ro1.strip())) == 0:
                    return "finding"
    ...
    def stig_rule_238303(self, ro):
        #print("response " + str(ro))
        if re.search("/sbin/auditctl p+i+n+u+g+s+b+acl+xattrs+sha512", ro):
    ...
    def stig_rule_238314(self, ro):
        if(len(ro.strip())) == 0:
            return "finding"
        #print("\033[1;33m--ITEMS IN CONFIG, BUT NOT SHOWING WITH COMMAND-------\033[0;0m")

        #if re.search("-a always,exit -F arch=b32 -S init_module -k modules", ro):
        #    if re.search("-a always,exit -F arch=b64 -S init_module -k modules", ro):

        if re.search("init_module -k modules", ro):
            if re.search("init_module -k modules", ro):
                return "not_finding"
            else: 
                return "finding"
